function sys = tbx_sm_model_depth_2p(p)

    Cz = [0 0 p.CzW/p.Vs p.CzQ*p.L/p.Vs p.CzB1 p.CzB2 0];
    Cm = [0 0 p.CmW/p.Vs p.CmQ*p.L/p.Vs p.CmB1 p.CmB2 0];

    Fz_navire = p.Vs^2/(2*p.CRemp*p.L)*Cz;
    My_navire = p.Vs^2/(2*p.CRemp)*Cm;

    Fz_couplage = [0 0 0       p.mu*p.Vs 0 0 0];
    My_couplage = [0 0 0 -p.mu*p.Xg*p.Vs 0 0 0];

    Fz_hydrostat = [0              0 0 0 0 0   (p.mu-1)*p.g];
    My_hydrostat = [0 -p.mu*p.Zg*p.g 0 0 0 0 -p.mu*p.Xg*p.g];
    ...
    thetapt = [0     0 0 1 0 0 0];
    Zpt     = [0 -p.Vs 1 0 0 0 0];
