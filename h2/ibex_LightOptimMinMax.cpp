...
LightOptimMinMax::LightOptimMinMax(NormalizedSystem& y_sys, Ctc& ctc_xy,UnconstrainedLocalSearch *local_solver,bool csp_actif):
                                                ...
                                                best_point_eval(y_sys.box), w(Interval(-3,1)) {
...
Interval LightOptimMinMax::eval_all(Function* f,const IntervalVector& box) {
        Interval eval;
        if(intg_eval)
                eval = eval_integral(box,1000);
        else
        {
                eval = f->eval(box);
                //cout << "constraint = " << eval << "box = " << box << endl;
        }
...
Interval LightOptimMinMax::eval_integral(const IntervalVector& box, int max_w_cell)
{
    cout << "box inside = " << box << endl;
    //cout << box.max_diam() << endl;

    max_w_cell = 1000;

    // cout << max_w_cell << endl;
    int n = box.size();
    ...
    Interval res = xy_sys.goal->eval(box_with_w);
    ...
    if (!first_res[0].is_unbounded())
    {
        count  = true;
        volume = first_res.volume();
    }
    ...
    while(!stop)
    {
        stop = stop_crit(iter,volume);
        IntervalVector box_max = w_heap.pop();
        volume -= box_max.volume();
        ...
        Interval inter_res_1 = xy_sys.goal->eval(box_1);
        Interval inter_res_2 = xy_sys.goal->eval(box_2);
...
bool LightOptimMinMax::stop_crit(int iter, double volume)
{
        int max_iter = 1000;
        int max_iter_midpoint = 5000;
        if (iter == max_iter && nb_iter != 0)
                return true;

        if (nb_iter == 0 && iter == max_iter_midpoint)
                return true;
...
Interval evaluate_surface(SortedHeap& w_heap)
{
    ...
    cout << "res integral = " << Interval(lb,ub) << endl;
